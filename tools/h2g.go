...
func (j *jvmtih) genJvmtiCWrapper(srcn string) {
	...
	// generate $srcn.h
	go func() {
		f, err := os.Create(srcn + ".h")
		if err != nil {
			fmt.Println("Failed to create file ", srcn)
			panic(err)
		}
		defer f.Close()
		...
		prn("#ifndef __", srcn, "_H__")
		prn("#define __", srcn, "_H__\n")
		prn("#ifdef __cplusplus\nextern \"C\" {\n#endif\n")
	...
	go func() {
		...
		cwr := func(s string) {
			...
			if err != nil {
				fmt.Println("Cannot write to ", fname, ", content:", s)
				panic(err)
			}
		}
		prt := func(ar ...string) {
		...
		for _, m := range j.apis {
			...
			proto := j.cwrapperFor(m)
			if m.varg {
				prn("// WARNING: variadic args not supported for now, bite me!")
